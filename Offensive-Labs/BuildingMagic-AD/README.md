...
The recovered Administrator hash was **reused** across the environment. Passing it against the final un-compromised user, **`a.flatch`**, granted a shell as **`a.flatch`** — an account BloodHound flags as **Tier Zero** (`Admin Count: TRUE`, member of the domain **Administrators** group), i.e. full Domain Admin–level control:
